...
def resize_avatar_for_web():
    ...
    # 檢查原始頭像是否存在
    avatar_path = "images/author.jpg"
    if not os.path.exists(avatar_path):
        print("❌ 找不到原始頭像文件: images/author.jpg")
        return
    ...
    try:
        ...
        # 定義網頁頭像使用的尺寸
        avatar_sizes = {
            "avatar_large": (120, 120),      # 大頭像 (About頁面)
            "avatar_medium": (80, 80),       # 中等頭像 (側邊欄)
            "avatar_small": (60, 60),        # 小頭像 (評論區)
            "avatar_navbar": (40, 40),       # 導航欄頭像
            "avatar_favicon": (32, 32)       # 極小頭像
        }
        ...
        for size_name, (width, height) in avatar_sizes.items():
            ...
            resized_avatar.save(filename, "JPEG", quality=85, optimize=True)
        ...
        # 創建一個專門用於網站的標準頭像 (替換原始文件)
        web_avatar = original_avatar.resize((80, 80), Image.Resampling.LANCZOS)
        web_avatar.save("images/author_web.jpg", "JPEG", quality=85, optimize=True)
        
        # 獲取文件大小
        web_file_size = os.path.getsize("images/author_web.jpg")
        web_file_size_kb = web_file_size / 1024
        
        print(f"\n✅ 網站標準頭像: 80x80px - {web_file_size_kb:.1f} KB")
        ...
        print(f"   - images/author_web.jpg (80x80px, {web_file_size_kb:.1f} KB)")
        
        print("\n💡 頭像使用建議：")
        print("   - author_large.jpg (120x120): About頁面使用")
        print("   - author_medium.jpg (80x80): 側邊欄使用")
        print("   - author_web.jpg (80x80): 通用網站頭像")
        print("   - author_small.jpg (60x60): 評論區使用")
        print("   - author_navbar.jpg (40x40): 導航欄使用")
